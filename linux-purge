...
program_version=1.0.158-7713436 \
...
Copyright (C) $copyright_years Jarno Ilari Suni
...
# Note: Depends on Ubuntu's package naming policy
get_latest_releases() {
	local meta_kernels; meta_kernels=$(get_meta_kernels) \
	&& dpkg-query -W -f '${Depends}\n${Pre-Depends}\n' $meta_kernels \
	| sed -r -e 's/ \([^)]*\)//g' -e 's/,? (\| )?/\n/g' \
	| sed -nr 's/^linux-(signed-)?image-(unsigned-)?([0-9]+\..+)/\3/p' \
...
get_latest_available_releases() {
	local meta_kernels; meta_kernels=$(get_meta_kernels) \
	&& apt-cache depends -i $meta_kernels \
	| sed -nr 's/^.*Depends: linux-(signed-)?image-(unsigned-)?([0-9]+\..+)/\3/p' \
	| linux-version sort | uniq
}
...
get_info() {
	...
	local -A avail
	local -r regex1=': linux-(signed-)?image-(unsigned-)*([[:digit:]]+\..*)'
	while IFS= read -r line; do
		[[ ${line:0:1} == l ]] && kernel=$line || {
			[[ $line =~ $regex1 ]] && avail[$kernel]="${BASH_REMATCH[3]}"
		}
	done < <(apt-cache depends --important $meta_kernels)

	local -A inst
	local -r regex2='([^ ]*).* (linux-(signed-)?image-(unsigned-)?([[:digit:]]+\.[^, ]*))'
	while IFS= read -r line; do
		[[ $line =~ $regex2 ]] && inst[${BASH_REMATCH[1]}]=${BASH_REMATCH[5]}
	done < <(dpkg-query -W -f '${Package} ${Depends} ${Pre-Depends}\n' $meta_kernels)

	local -a meta; mapfile -t meta <<<"$meta_kernels"

	local -r format='%-28s %-25s %-25s\n'
	printf "$format" 'Installed Meta-Kernel:' 'Currently Available:' 'Installed:'
	for i in ${!meta[*]}; do
		metapkg=${meta[$i]}
...
clear_boot() {
	...
	sed -nr 's/^linux-image-(unsigned-)?([0-9]+\..+)/\2/p')
	# Assume filenames at /boot are nice i.e. do not contain newline etc.
...
free_some_inodes() {
	...
	sed -rn 's/^.i  linux-headers-([0-9.]+(-[^-]+)?)$/\1/p' | linux-version sort)
...
you may have to reinstall the package by 'apt-get install --reinstall'."
	...
	Releases=$(dpkg-query -W -f='${db:Status-Abbrev} ${Package}\n' |
	 sed -nr 's/^[ih]i  linux-image-(unsigned-)?([0-9]+\..+)/\2/p' | linux-version sort)
	...
	if [[ -z $keep ]]; then
		...
		[[ $LatestReleases ]] || error "There are no meta-kernels so \
can not decide which kernels to keep."
...
params=$(env -u GETOPT_COMPATIBLE getopt -o abcd::fhi:k:lmnostrvy -l \
...
[[ $interface = d ]] && {
	hash dialog 2>/dev/null || {
		interface=w
		warn 'Using whiptail, if needed, because dialog is not installed.'
	}
}
[[ $interface = w ]] && {
...
if [[ $debug ]]; then
	...
	operate
	note "If you find this software useful, please donate. You can \
